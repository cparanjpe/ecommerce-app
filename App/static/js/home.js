...
const handleSearchChange=async(e)=>{
    ...
    var oneCategory = []
    data.forEach(ele =>{
        if(ele.substring(0, 9) === "category_"){
            oneCategory.push( ele.substring(9))
            
        }
    })
    // console.log(oneCategory)
    oneCategory.forEach(cat =>{
        fetchFromAllCat(cat,e.target.value)
    })
    }else{
        renderHomePage()
    }
...
const fetchFromAllCat = async(anyCat,search) =>{
    const res1 = await fetch(`${window.location.origin}/api/fetch_category/${anyCat}`)
    const data1 = await res1.json();
    // console.log(data1)
    let dataStr = `<div id="subContainer" class="flex flex-wrap justify-center mx-auto">`
    ...
    data1.forEach(ele =>{
        ...
        if(ele.name.toLowerCase().includes(search.toLowerCase())){
            ...
            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <a href="/details/${anyCat}/${ele.id}" type="button" class="cursor-pointer text-black mt-3 bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-bold rounded-full text-sm px-3 py-1 text-center mr-2 mb-2 dark:focus:ring-yellow-900">View Details</a>
                    <div class="mt-3">
